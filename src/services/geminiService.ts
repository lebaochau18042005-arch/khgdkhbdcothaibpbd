...
function autoAlignCompetencyForInstructionalLesson(row: any, grade: string = "10", subject: string = ""): any {
  ...
  if (aiMissing) {
    if (isEnglish) {
      ...
      const isDataOrSim = /số liệu|biểu đồ|bảng số liệu|bản đồ|gis|thống kê|tính toán|mô phỏng|thuật toán|dữ liệu|cơ cấu|gdp|gni/i.test(combined);
