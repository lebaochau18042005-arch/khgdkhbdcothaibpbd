...
function autoAlignCompetencyForInstructionalLesson(row: any, grade: string = "10", subject: string = ""): any {
  ...
  if (isSocialMissing) {
    if (/biển|đảo|hải đảo|hoàng sa|trường sa|vùng biển|thềm lục địa|unclos|hàng hải|kinh tế biển/i.test(combined)) {
      ...
      const isFull = /thiên tai|bão|ngập lụt/i.test(cleanLessonName.toLowerCase());
      const level = isFull ? "Tích hợp toàn phần" : "Tích hợp bộ phận";
      ...
      const isFull = /biến đổi khí hậu|khí quyển|khí hậu/i.test(cleanLessonName.toLowerCase());
      const level = isFull ? "Tích hợp toàn phần" : "Tích hợp bộ phận";
      ...
      const isFull = /môi trường|tài nguyên|bảo vệ/i.test(cleanLessonName.toLowerCase());
      const level = isFull ? "Tích hợp toàn phần" : "Tích hợp bộ phận";
