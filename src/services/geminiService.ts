...
function autoAlignCompetencyForInstructionalLesson(row: any, grade: string = "10", subject: string = ""): any {
  ...
  if (nlsMissing) {
    if (isEnglish) {
      ...
      if (/bản đồ số|atlat điện tử|gis|google earth|hệ thống thông tin địa lí|phần mềm mô phỏng|định vị|gps|viễn thám/i.test(combined)) {
      ...
      } else if (/vẽ biểu đồ|xử lý số liệu|bảng số liệu|tính toán|tốc độ tăng trưởng|cơ cấu|thực hành|báo cáo|dự án|tuyên truyền|sản phẩm|infographic|áp phích|poster|thuyết trình/i.test(combined)) {
