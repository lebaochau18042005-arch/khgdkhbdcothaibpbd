...
function autoAlignCompetencyForInstructionalLesson(row: any, grade: string = "10", subject: string = ""): any {
  ...
  // 1. NĂNG LỰC SỐ (NLS - TT 02 & CV 3456)
  ...
  if (nlsMissing) {
    if (isEnglish) {
      if (/getting started/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.1.NC1a; Thành phần NLS: Duyệt, tìm kiếm và lọc dữ liệu số\n- YCCĐ NLS: Tìm kiếm và khai thác hình ảnh, tư liệu âm thanh/video số về chủ đề ${topicLabel} qua môi trường số.\n- Hành vi HS: Sử dụng công cụ tìm kiếm số để thu thập từ vựng và hình ảnh mở đầu cho ${topicLabel}.\n- Sản phẩm đầu ra: Bộ sưu tập từ vựng, hình ảnh số theo chủ đề ${topicLabel}.`;
      } else if (/language|pronunciation|grammar|vocabulary/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 6.2.NC1a; Thành phần NLS: Sử dụng phần mềm và công nghệ số chuyên ngành\n- YCCĐ NLS: Sử dụng phần mềm số và từ điển trực tuyến để rèn luyện phát âm và tra cứu cấu trúc ngôn ngữ trong bài ${cleanLessonName}.\n- Hành vi HS: Tra cứu ngữ âm, phát âm mẫu trên ứng dụng từ điển số và ghi âm đối chiếu.\n- Sản phẩm đầu ra: File ghi âm phát âm chuẩn và bảng tra cứu từ vựng số ${topicLabel}.`;
      } else if (/reading/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.2.NC1a; Thành phần NLS: Đánh giá dữ liệu, thông tin và nội dung số\n- YCCĐ NLS: Đánh giá độ tin cậy của thông tin số và khai thác tư liệu đọc mở rộng ${topicLabel} bằng tiếng Anh.\n- Hành vi HS: Đọc hiểu và phân tích độ tin cậy của các bài báo, tư liệu tiếng Anh trên internet về ${cleanLessonName}.\n- Sản phẩm đầu ra: Bản tóm tắt bài đọc kèm danh mục nguồn tin kiểm chứng.`;
      } else if (/speaking/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 2.2.NC1a; Thành phần NLS: Chia sẻ thông tin và nội dung số\n- YCCĐ NLS: Chia sẻ bản ghi âm, clip thuyết trình tiếng Anh ${topicLabel} qua nền tảng số của lớp.\n- Hành vi HS: Ghi hình/ghi âm bài nói tiếng Anh và đăng tải lên không gian học tập trực tuyến.\n- Sản phẩm đầu ra: Video/audio thuyết trình tiếng Anh ${topicLabel} được số hóa hoàn chỉnh.`;
      } else if (/listening/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.1.NC1a; Thành phần NLS: Duyệt, tìm kiếm và lọc dữ liệu số\n- YCCĐ NLS: Tiếp nhận, xử lý và điều hướng giữa các nguồn học liệu âm thanh số trong học tập bài ${cleanLessonName}.\n- Hành vi HS: Nghe podcast/audio số, điều chỉnh tốc độ nghe và ghi chú thông tin then chốt của bài ${cleanLessonName}.\n- Sản phẩm đầu ra: Bản ghi chép (note-taking) số hóa các ý chính từ bài nghe.`;
      } else if (/writing/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 3.1.NC1a; Thành phần NLS: Phát triển nội dung số\n- YCCĐ NLS: Tạo lập và định dạng bài viết đoạn văn, bài luận tiếng Anh ${topicLabel} bằng công cụ soạn thảo số.\n- Hành vi HS: Soạn thảo bài viết, sử dụng công cụ kiểm tra chính tả/ngữ pháp và định dạng văn bản số.\n- Sản phẩm đầu ra: Bài luận tiếng Anh ${topicLabel} được số hóa và trình bày chuẩn mực.`;
      } else if (/project|looking back/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 2.4.NC1a; Thành phần NLS: Hợp tác thông qua công nghệ số\n- YCCĐ NLS: Hợp tác nhóm trực tuyến trên nền tảng số để thiết kế sản phẩm dự án học tập ${topicLabel}.\n- Hành vi HS: Phân công nhiệm vụ nhóm qua bảng số, cùng chỉnh sửa slide thuyết trình dự án.\n- Sản phẩm đầu ra: Bộ slide thuyết trình dự án hoàn chỉnh của nhóm trên không gian số.`;
      } else {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.1.NC1a; Thành phần NLS: Duyệt, tìm kiếm và lọc dữ liệu số\n- YCCĐ NLS: Khai thác dữ liệu, thông tin số chính thống phục vụ nhiệm vụ học tập ${topicLabel}.\n- Hành vi HS: Tìm kiếm và chọn lọc tư liệu số liên quan đến bài học ${cleanLessonName}.\n- Sản phẩm đầu ra: Phiếu học tập số hóa ghi nhận thông tin đã tìm kiếm.`;
      }
    } else {
      // Non-English subjects (Địa lí, Lịch sử, Toán, Lý, Hóa, Sinh, Tin học, Công nghệ, GDCD...)
      if (/vị trí|bản đồ|atlat|gis|google earth|hệ thống thông tin địa lí|định vị|gps|viễn thám/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 6.2.NC1a; Thành phần NLS: Sử dụng phần mềm và công nghệ số chuyên ngành\n- YCCĐ NLS: Khai thác và sử dụng phần mềm bản đồ số, GIS hoặc Atlat điện tử để tra cứu, phân tích không gian và thể hiện đối tượng địa lí ${topicLabel}.\n- Hành vi HS: Thao tác trên phần mềm bản đồ số/Google Earth/Atlat trực tuyến để xác định vị trí, đo đạc khoảng cách và phân tích phân bố không gian ${topicLabel}.\n- Sản phẩm đầu ra: Ảnh chụp màn hình/bản đồ số đã được học sinh chú giải và phân tích hoàn chỉnh.`;
      } else if (/thực hành|vẽ biểu đồ|thiết kế infographic|sản xuất video|áp phích|poster/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 3.1.NC1a; Thành phần NLS: Phát triển nội dung số\n- YCCĐ NLS: Tạo lập, biên tập và định dạng sản phẩm số (bảng tính số liệu, biểu đồ số, infographic hoặc báo cáo thực hành) ${topicLabel}.\n- Hành vi HS: Sử dụng công cụ bảng tính/đồ họa/soạn thảo số để tính toán số liệu, vẽ biểu đồ và thiết kế báo cáo thực hành ${topicLabel}.\n- Sản phẩm đầu ra: Báo cáo thực hành/biểu đồ số/infographic được số hóa và chia sẻ trên nền tảng học tập của lớp.`;
      } else if (/môi trường|tài nguyên|biến đổi khí hậu|sinh quyển|rừng|năng lượng|phát triển bền vững|xanh/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 4.2.NC1a; Thành phần NLS: Bảo vệ môi trường và sức khỏe trong môi trường số\n- YCCĐ NLS: Nhận thức và đánh giá tác động của công nghệ số, tiêu thụ năng lượng thiết bị số đến môi trường và phát triển bền vững trong bài ${cleanLessonName}.\n- Hành vi HS: Tra cứu và phân tích dữ liệu môi trường số, đề xuất giải pháp xanh gắn với bài học ${cleanLessonName}.\n- Sản phẩm đầu ra: Sơ đồ tư duy/bài thu hoạch số đánh giá tác động môi trường và phát triển bền vững.`;
      } else if (/đô thị|vùng kinh tế|ngành kinh tế|nông nghiệp|công nghiệp|dịch vụ|dân cư|lao động|thương mại|du lịch|giao thông/i.test(combined)) {
        if (/hợp tác|thảo luận|nhóm|dự án/i.test(combined)) {
          row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 2.4.NC1a; Thành phần NLS: Hợp tác thông qua công nghệ số\n- YCCĐ NLS: Hợp tác nhóm trực tuyến thông qua công cụ và nền tảng số để hoàn thành nhiệm vụ học tập ${topicLabel}.\n- Hành vi HS: Phối hợp làm việc nhóm trên tài liệu số dùng chung, thảo luận và phản hồi trực tuyến về ${cleanLessonName}.\n- Sản phẩm đầu ra: Sản phẩm học tập hợp tác nhóm ${topicLabel} được số hóa hoàn chỉnh.`;
        } else {
          row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.2.NC1a; Thành phần NLS: Đánh giá dữ liệu, thông tin và nội dung số\n- YCCĐ NLS: Đánh giá dữ liệu, thông tin và nội dung số; kiểm chứng chéo độ tin cậy của các nguồn học liệu số ${topicLabel}.\n- Hành vi HS: Tra cứu nhiều nguồn tư liệu số chính thống, đối chiếu và thẩm định độ chính xác của thông tin ${topicLabel}.\n- Sản phẩm đầu ra: Bảng đối chiếu và thẩm định độ tin cậy của các nguồn dữ liệu số ${topicLabel}.`;
        }
      } else if (/chia sẻ|đăng tải|truyền thông|kết nối/i.test(combined)) {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 2.2.NC1a; Thành phần NLS: Chia sẻ thông tin và nội dung số\n- YCCĐ NLS: Chia sẻ dữ liệu, tư liệu số và kết quả bài học qua không gian học tập trực tuyến của lớp.\n- Hành vi HS: Đăng tải tài liệu, kết quả làm việc số lên Padlet/LMS của lớp phục vụ chia sẻ tri thức.\n- Sản phẩm đầu ra: Sản phẩm học tập được lưu trữ và chia sẻ công khai trên nền tảng số.`;
      } else {
        row.digitalCompetencyTT02 = `Mã chỉ báo NLS: 1.1.NC1a; Thành phần NLS: Duyệt, tìm kiếm và lọc dữ liệu số\n- YCCĐ NLS: Tìm kiếm, duyệt và lọc dữ liệu, thông tin số chính thống phục vụ yêu cầu cần đạt ${topicLabel}.\n- Hành vi HS: Sử dụng từ khóa logic để tra cứu văn bản, biểu đồ, hình ảnh số chính thống trên internet phục vụ ${cleanLessonName}.\n- Sản phẩm đầu ra: Bộ dữ liệu số tổng hợp thông tin cốt lõi phục vụ bài học ${cleanLessonName}.`;
      }
    }
  }
  ...
  if (aiMissing) {
    if (isEnglish) {
      if (/getting started/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: ${g}; Chủ đề: C3; Mã chỉ báo NL AI: NLc-${g}.C3.1\n- Yêu cầu cần đạt AI: Kỹ thuật Prompt Engineering cơ bản (Sử dụng prompt để AI gợi ý từ vựng, ngữ cảnh giao tiếp theo chủ đề ${topicLabel}).\n- Hành vi học sinh: Học sinh nhập câu lệnh prompt có bối cảnh để AI gợi ý từ vựng và câu đàm thoại ${topicLabel}; đối chiếu với SGK.\n- Sản phẩm đầu ra: Lịch sử câu lệnh prompt và bảng từ vựng gợi ý đã được kiểm chứng.\n- Tiêu chí đánh giá: Câu lệnh rõ ràng, thông tin từ vựng chính xác và phù hợp chủ đề.`;
      } else if (/language|pronunciation|grammar|vocabulary/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: ${g}; Chủ đề: C2; Mã chỉ báo NL AI: NLc-${g}.C2.1\n- Yêu cầu cần đạt AI: Ứng dụng AI trong học tập ngôn ngữ (Sử dụng AI giải thích sắc thái từ vựng, cấu trúc ngữ pháp trong bài ${cleanLessonName}).\n- Hành vi học sinh: Học sinh yêu cầu AI phân biệt các cấu trúc ngữ pháp tương đồng và đưa ra ví dụ minh họa cho ${cleanLessonName}.\n- Sản phẩm đầu ra: Bảng so sánh ngữ pháp/sắc thái từ vựng do AI gợi ý và học sinh chỉnh sửa.\n- Tiêu chí đánh giá: Giải thích đúng ngữ pháp, có ví dụ đối chiếu chuẩn xác với SGK.`;
      } else if (/reading/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: ${g}; Chủ đề: A3; Mã chỉ báo NL AI: NLa-${g}.A3.1\n- Yêu cầu cần đạt AI: Kiểm soát và giám sát AI (Đọc hiểu văn bản SGK và kiểm chứng tính chính xác của bản tóm tắt do AI tạo ra về ${cleanLessonName}).\n- Hành vi học sinh: Học sinh yêu cầu AI tóm tắt văn bản đọc, sau đó rà soát và chỉ ra các điểm AI tóm tắt chưa chuẩn hoặc ảo giác.\n- Sản phẩm đầu ra: Bản nhận xét đối chiếu văn bản tóm tắt của AI với nội dung bài đọc SGK.\n- Tiêu chí đánh giá: Phát hiện chính xác điểm chưa chuẩn xác của AI, lập luận dựa trên dẫn chứng bài đọc.`;
      } else if (/speaking/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: ${g}; Chủ đề: C2; Mã chỉ báo NL AI: NLc-${g}.C2.1\n- Yêu cầu cần đạt AI: Ứng dụng AI tương tác đàm thoại (Luyện tập phản xạ giao tiếp tiếng Anh theo chủ đề ${topicLabel}).\n- Hành vi học sinh: Học sinh tương tác thoại hoặc chat với AI chatbot đóng vai nhân vật trong tình huống giao tiếp về ${cleanLessonName}.\n- Sản phẩm đầu ra: Bản ghi chép/lịch sử đoạn hội thoại tiếng Anh với AI.\n- Tiêu chí đánh giá: Phản xạ ngôn ngữ tự nhiên, sử dụng đúng từ vựng và cấu trúc của bài học.`;
      } else if (/listening/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: ${g}; Chủ đề: A1; Mã chỉ báo NL AI: NLa-${g}.A1.1\n- Yêu cầu cần đạt AI: Con người trong hệ thống AI (Đánh giá độ chính xác của phụ đề do AI nhận diện khi nghe bài ${cleanLessonName}).\n- Hành vi học sinh: Học sinh nghe audio gốc và kiểm tra lại transcript/phụ đề tự động do AI sinh ra.\n- Sản phẩm đầu ra: Bản transcript đã được học sinh nghe và đính chính các lỗi nhận diện của AI.\n- Tiêu chí đánh giá: Sửa đúng các từ vựng AI nhận diện sai, hiểu chính xác nội dung audio.`;
      } else if (/writing/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLb - Đạo đức AI, an toàn, pháp luật và trách nhiệm; Khối lớp: ${g}; Chủ đề: B3; Mã chỉ báo NL AI: NLb-${g}.B3.1\n- Yêu cầu cần đạt AI: Liêm chính học thuật và trách nhiệm khi sử dụng AI (Ghi rõ nguồn, không sao chép nguyên văn khi viết bài ${cleanLessonName}).\n- Hành vi học sinh: Sử dụng AI để gợi ý dàn ý, tự viết bài luận và lập bảng minh bạch nội dung AI gợi ý.\n- Sản phẩm đầu ra: Bài viết hoàn chỉnh ${topicLabel} kèm phần chú thích minh bạch mức độ sử dụng AI.\n- Tiêu chí đánh giá: Tự thể hiện văn phong cá nhân, tuyệt đối không sao chép nguyên văn từ AI, có trích dẫn rõ ràng.`;
      } else if (/project|looking back/i.test(combined)) {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLd - Thiết kế, thử nghiệm và cải tiến hệ thống AI; Khối lớp: ${g}; Chủ đề: D1; Mã chỉ báo NL AI: NLd-${g}.D1.1\n- Yêu cầu cần đạt AI: Thiết kế dự án học tập có ứng dụng AI (Thiết kế bài thuyết trình dự án có sự hỗ trợ công cụ AI cho ${cleanLessonName}).\n- Hành vi học sinh: Sử dụng AI để lên ý tưởng trình bày, hỗ trợ thiết kế slide và chuẩn bị nội dung thuyết trình ${topicLabel}.\n- Sản phẩm đầu ra: Hồ sơ dự án học tập có ứng dụng AI và được kiểm chứng bởi học sinh.\n- Tiêu chí đánh giá: Ý tưởng sáng tạo, nội dung tiếng Anh chuẩn xác, minh bạch công cụ AI sử dụng.`;
      } else {
        row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: ${g}; Chủ đề: C3; Mã chỉ báo NL AI: NLc-${g}.C3.1\n- Yêu cầu cần đạt AI: Kỹ thuật Prompt Engineering (Đặt câu lệnh có cấu trúc để khai thác tư liệu học tập có chọn lọc cho bài ${cleanLessonName}).\n- Hành vi học sinh: Thiết lập câu lệnh truy vấn thông tin học tập, phân tích và chọn lọc kết quả từ AI.\n- Sản phẩm đầu ra: Phiếu học tập ghi nhận câu lệnh prompt và kết quả chọn lọc ${topicLabel}.\n- Tiêu chí đánh giá: Prompt có cấu trúc rõ ràng, thông tin thu nhận có tính chọn lọc và chính xác.`;
      }
    } else {
      // Non-English subjects (Địa lí, Lịch sử, Toán, Lý, Hóa, Sinh, Tin học, Công nghệ, GDCD...)
      if (g === "12") {
        if (/vị trí địa lí|lãnh thổ|biển đông|vùng biển|hải đảo|chủ quyền|an ninh quốc phòng|biên giới/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLb - Đạo đức AI, an toàn, pháp luật và trách nhiệm; Khối lớp: 12; Chủ đề: B2; Mã chỉ báo NL AI: 12.B2.1\n- Yêu cầu cần đạt AI: Nhận diện được các nguy cơ deepfake, tin giả, thông tin sai lệch do AI tạo ra; thực hiện được các biện pháp xác thực nguồn tin đa kênh khi tìm hiểu bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh nhận diện nguy cơ thông tin sai lệch/deepfake từ AI về ${cleanLessonName}, thực hiện kiểm chứng chéo với cổng thông tin chính thống và SGK để bảo vệ chủ quyền dữ liệu.\n- Sản phẩm đầu ra: Bản báo cáo tư liệu ${topicLabel} có kèm bảng đối chiếu xác thực đa nguồn và ghi chú kiểm chứng tin giả.\n- Tiêu chí đánh giá: Đảm bảo tính xác thực tuyệt đối của nguồn tin, bảo vệ chủ quyền thông tin quốc gia.`;
        } else if (/khí hậu|gió mùa|nhiệt đới ẩm|thiên tai|bão|lũ|ngập|hạn mặn|sạt lở|biến đổi khí hậu|đồng bằng sông cửu long|xâm nhập mặn|thuận thiên/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 12; Chủ đề: C3; Mã chỉ báo NL AI: 12.C3.1\n- Yêu cầu cần đạt AI: Sử dụng thành thạo các mô hình AI chuyên ngành để phân tích số liệu khí tượng, mô hình thích ứng biến đổi khí hậu và cảnh báo thiên tai trong bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh thiết lập câu lệnh prompt truy vấn mô hình AI để xử lý số liệu khí hậu, phân tích kịch bản thích ứng BĐKH thuận thiên cho ${cleanLessonName}.\n- Sản phẩm đầu ra: Bảng số liệu thống kê, biểu đồ phân tích và kết quả nhận xét giải pháp thích ứng ${topicLabel} được kiểm chứng.\n- Tiêu chí đánh giá: Số liệu chính xác, biểu đồ trực quan, đối chiếu chuẩn xác với Atlat/SGK.`;
        } else if (/tài nguyên|suy giảm tài nguyên|đa dạng sinh học|môi trường|bền vững|rừng|sinh quyển|thổ nhưỡng|năng lượng xanh/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLb - Đạo đức AI, an toàn, pháp luật và trách nhiệm; Khối lớp: 12; Chủ đề: B3; Mã chỉ báo NL AI: 12.B3.1\n- Yêu cầu cần đạt AI: Đánh giá được tác động của việc tiêu thụ năng lượng và tài nguyên tính toán của AI đối với môi trường và phát triển bền vững khi học bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh phân tích tác động môi trường từ hạ tầng AI và đề xuất giải pháp sử dụng công nghệ xanh, bền vững gắn với nội dung ${cleanLessonName}.\n- Sản phẩm đầu ra: Báo cáo ngắn hoặc sơ đồ tư duy đánh giá tác động môi trường của AI trong môn học.\n- Tiêu chí đánh giá: Lập luận có căn cứ khoa học, đề xuất giải pháp khả thi gắn với bảo vệ môi trường.`;
        } else if (/đông nam bộ|đô thị hóa|đô thị thông minh|trung tâm tài chính|công nghệ cao|khu kinh tế|dự án sáng tạo/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 12; Chủ đề: A1; Mã chỉ báo NL AI: 12.A1.MR1\n- Yêu cầu cần đạt AI: Thực hiện được việc phân tích quyền kiểm soát và trách nhiệm của con người trong vòng đời AI thông qua một dự án sáng tạo AI bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh ứng dụng AI để tổng hợp dữ liệu, thiết kế đồ họa/báo cáo thực hành ${cleanLessonName}, thuyết minh và kiểm chứng kết quả với số liệu thực tế.\n- Sản phẩm đầu ra: Báo cáo thực hành/dự án số hoàn chỉnh ${topicLabel} có minh bạch phần đóng góp của AI và phần tự làm của học sinh.\n- Tiêu chí đánh giá: Đúng chuẩn kiến thức môn học, minh bạch lịch sử prompt, có số liệu đối chiếu kiểm chứng rõ ràng.`;
        } else if (/kinh tế|nông nghiệp|công nghiệp|dịch vụ|du lịch|giao thông|bắc bộ|bắc trung bộ|nam trung bộ|tây nguyên|sông hồng|vùng/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 12; Chủ đề: C3; Mã chỉ báo NL AI: 12.C3.1\n- Yêu cầu cần đạt AI: Sử dụng thành thạo các mô hình AI chuyên ngành để phân tích cấu trúc kinh tế không gian, bảng số liệu và vẽ biểu đồ tăng trưởng bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh thiết lập câu lệnh prompt truy vấn mô hình AI chuyên ngành để xử lý số liệu, phân tích cơ cấu kinh tế theo ngành và lãnh thổ ${topicLabel}.\n- Sản phẩm đầu ra: Bảng số liệu thống kê, biểu đồ phân tích và kết quả nhận xét ${topicLabel} được kiểm chứng.\n- Tiêu chí đánh giá: Số liệu chính xác, biểu đồ trực quan, đối chiếu chuẩn xác với Atlat/SGK.`;
        } else if (/dân cư|dân số|lao động|việc làm|nghề nghiệp|thị trường lao động/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 12; Chủ đề: A2; Mã chỉ báo NL AI: 12.A2.1\n- Yêu cầu cần đạt AI: Xác định được lộ trình phát triển năng lực bản thân để thích ứng và cộng tác hiệu quả với AI trong các ngành nghề liên quan đến "${cleanLessonName}".\n- Hành vi học sinh: Học sinh sử dụng AI để khảo sát xu hướng chuyển dịch cơ cấu lao động, tự đánh giá điểm mạnh và lập kế hoạch thích ứng nghề nghiệp.\n- Sản phẩm đầu ra: Bản kế hoạch phát triển năng lực cá nhân trong kỷ nguyên AI.\n- Tiêu chí đánh giá: Mục tiêu rõ ràng, định hướng nghề nghiệp thực tế, chủ động làm chủ công nghệ.`;
        } else if (/thực hành|báo cáo|dự án|infographic|áp phích/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLd - Thiết kế, thử nghiệm và cải tiến hệ thống AI; Khối lớp: 12; Chủ đề: D1; Mã chỉ báo NL AI: 12.D1.1\n- Yêu cầu cần đạt AI: Xây dựng và thực hiện báo cáo thực hành địa lí hoàn chỉnh có ứng dụng AI từ bước thu thập dữ liệu, xử lý, phân tích đến thuyết trình bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh ứng dụng AI để tổng hợp dữ liệu, thiết kế đồ họa/báo cáo thực hành ${cleanLessonName}, thuyết minh và kiểm chứng kết quả với số liệu thực tế.\n- Sản phẩm đầu ra: Báo cáo thực hành/dự án số hoàn chỉnh ${topicLabel} có minh bạch phần đóng góp của AI và phần tự làm của học sinh.\n- Tiêu chí đánh giá: Đúng chuẩn kiến thức môn học, minh bạch lịch sử prompt, có số liệu đối chiếu kiểm chứng rõ ràng.`;
        } else {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 12; Chủ đề: A1; Mã chỉ báo NL AI: 12.A1.1\n- Yêu cầu cần đạt AI: Thể hiện được tư duy độc lập và năng lực làm chủ công nghệ; đưa ra phán đoán phản biện và quyết định cuối cùng khi nghiên cứu bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh đối chiếu các thông tin gợi ý của AI với kiến thức SGK/Atlat về ${cleanLessonName}, phản biện và đưa ra kết luận độc lập.\n- Sản phẩm đầu ra: Bản ghi chép bài học ${topicLabel} có tích hợp nhận xét phản biện câu trả lời của AI kèm căn cứ SGK.\n- Tiêu chí đánh giá: Thể hiện tư duy độc lập, con người làm chủ quyết định, không sao chép thụ động.`;
        }
      } else if (g === "11") {
        if (/hoa kỳ|nga|nhật bản|trung quốc|nam phi|eu|asean|khu vực|quốc gia/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 11; Chủ đề: C5; Mã chỉ báo NL AI: 11.C5.MR1\n- Yêu cầu cần đạt AI: Trình bày được kiến thức cơ bản về mạng nơ-ron nhân tạo và xử lý dữ liệu thống kê kinh tế - xã hội bài "${cleanLessonName}".\n- Hành vi học sinh: Nhập dữ liệu bảng vào AI để lọc, tính toán tốc độ tăng trưởng/cơ cấu và vẽ biểu đồ minh họa ${topicLabel}.\n- Sản phẩm đầu ra: Báo cáo phân tích dữ liệu thống kê kèm biểu đồ số do AI hỗ trợ cho bài ${cleanLessonName}.\n- Tiêu chí đánh giá: Tính toán chính xác, nhận xét rút ra có căn cứ khoa học.`;
        } else if (/định kiến|bias|công bằng|đạo đức/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLb - Đạo đức AI, an toàn, pháp luật và trách nhiệm; Khối lớp: 11; Chủ đề: B3; Mã chỉ báo NL AI: 11.B3.MR1\n- Yêu cầu cần đạt AI: Xác định và sơ đồ hoá được các vấn đề đạo đức có thể phát sinh trong từng bước thiết kế và vận hành AI bài "${cleanLessonName}".\n- Hành vi học sinh: Học sinh sơ đồ hóa các vấn đề đạo đức và rủi ro thiên lệch thông tin khi dùng AI học bài ${cleanLessonName}.\n- Sản phẩm đầu ra: Bảng sơ đồ hóa các vấn đề đạo đức AI kèm phương án kiểm chứng.\n- Tiêu chí đánh giá: Phân tích khách quan, chỉ rõ góc nhìn thiên kiến và căn cứ đối chiếu chuẩn.`;
        } else if (/thực hành|báo cáo|dự án|biên tập/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 11; Chủ đề: C2; Mã chỉ báo NL AI: 11.C2.MR1\n- Yêu cầu cần đạt AI: Sử dụng được công cụ AI để tạo và biên tập nội dung học liệu phục vụ học tập và đánh giá sản phẩm bài "${cleanLessonName}".\n- Hành vi học sinh: Thiết kế các bước thực hiện nhiệm vụ học tập có ứng dụng AI và tự đánh giá qua rubric cho bài ${cleanLessonName}.\n- Sản phẩm đầu ra: Quy trình giải pháp học tập tích hợp AI hoàn chỉnh ${topicLabel}.\n- Tiêu chí đánh giá: Quy trình logic, có tiêu chí kiểm thử kết quả và minh chứng rõ ràng.`;
        } else if (/prompt|câu lệnh|kỹ thuật/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 11; Chủ đề: C3; Mã chỉ báo NL AI: 11.C3.MR1\n- Yêu cầu cần đạt AI: Vận dụng được một số kỹ thuật prompt nâng cao như: ràng buộc định dạng đầu ra, chia nhỏ nhiệm vụ cho bài "${cleanLessonName}".\n- Hành vi học sinh: Đặt câu lệnh prompt có cấu trúc nâng cao để thu thập tư liệu bài học ${cleanLessonName}.\n- Sản phẩm đầu ra: Bảng ghi chú các prompt nâng cao và sản phẩm đã biên tập.\n- Tiêu chí đánh giá: Prompt có cấu trúc chuẩn, kết quả AI trả về chính xác.`;
        } else if (isCareer) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 11; Chủ đề: A2; Mã chỉ báo NL AI: 11.A2.1\n- Yêu cầu cần đạt AI: Đánh giá sự thay đổi ngành nghề dưới tác động của AI; xác định kỹ năng cần trau dồi liên quan đến "${cleanLessonName}".\n- Hành vi học sinh: Tra cứu và phân tích tác động của AI đến các ngành nghề liên quan đến ${cleanLessonName}.\n- Sản phẩm đầu ra: Bài thu hoạch phân tích cơ hội và thách thức nghề nghiệp trong thời đại AI.\n- Tiêu chí đánh giá: Đánh giá đa chiều, xác định đúng các kỹ năng cốt lõi con người cần có.`;
        } else {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 11; Chủ đề: A1; Mã chỉ báo NL AI: 11.A1.1\n- Yêu cầu cần đạt AI: Phân tích vai trò bổ trợ của AI; giải thích tại sao quyết định nhân văn và phán đoán thuộc về con người khi học "${cleanLessonName}".\n- Hành vi học sinh: Học sinh thảo luận nhóm về các phương án do AI đề xuất và đưa ra lựa chọn có trách nhiệm.\n- Sản phẩm đầu ra: Biên bản thảo luận nhóm khẳng định vai trò quyết định của con người đối với ${cleanLessonName}.\n- Tiêu chí đánh giá: Lập luận chặt chẽ, khẳng định con người làm chủ công nghệ.`;
        }
      } else {
        // Grade 10
        if (/bản đồ|gis|vũ trụ|hệ mặt trời|trái đất|thạch quyển|khí quyển|thủy quyển|thổ nhưỡng|sinh quyển/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 10; Chủ đề: C3; Mã chỉ báo NL AI: 10.C3.MR1\n- Yêu cầu cần đạt AI: Trình bày được ví dụ mô tả một số công nghệ để thiết kế và tạo AI khi tìm hiểu quy luật tự nhiên bài "${cleanLessonName}".\n- Hành vi học sinh: Thực hành đặt câu lệnh prompt có bối cảnh, vai trò và yêu cầu cụ thể để AI hỗ trợ tra cứu, phân tích quy luật địa lí bài ${cleanLessonName}.\n- Sản phẩm đầu ra: Bảng ghi nhận các câu lệnh prompt và kết quả phản hồi đã được tinh chỉnh ${topicLabel}.\n- Tiêu chí đánh giá: Câu lệnh rõ ràng, mạch lạc, kết quả AI trả về đạt yêu cầu.`;
        } else if (/cơ cấu kinh tế|gdp|gni|nông nghiệp|công nghiệp|dịch vụ|địa lí ngành/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 10; Chủ đề: C2; Mã chỉ báo NL AI: 10.C2.MR1\n- Yêu cầu cần đạt AI: Xác định được các yêu cầu cần có đối với việc ứng dụng AI thực hiện nhiệm vụ cụ thể bài "${cleanLessonName}".\n- Hành vi học sinh: Nhập dữ liệu bảng tính để AI hỗ trợ phân tích cơ cấu GDP/GNI của ${cleanLessonName}, sau đó đối chiếu lại với SGK/số liệu Tổng cục Thống kê.\n- Sản phẩm đầu ra: Bảng số liệu đã xử lý và biểu đồ cơ cấu kinh tế được kiểm chứng.\n- Tiêu chí đánh giá: Tính toán chính xác, vẽ biểu đồ chuẩn quy cách.`;
        } else if (/môi trường|phát triển bền vững|tài nguyên|dân số|đô thị hóa|dữ liệu/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLc - Các kĩ thuật và ứng dụng AI; Khối lớp: 10; Chủ đề: C4; Mã chỉ báo NL AI: 10.C4.MR1\n- Yêu cầu cần đạt AI: Phân tích được các dạng dữ liệu (hình ảnh, âm thanh, từ ngữ,...) được sử dụng để huấn luyện AI trong bài "${cleanLessonName}".\n- Hành vi học sinh: Ghi chú rõ nguồn tài liệu tham khảo và câu lệnh prompt đã sử dụng trong bài tập ${cleanLessonName}.\n- Sản phẩm đầu ra: Bài làm có phần trích dẫn nguồn AI đúng chuẩn mực.\n- Tiêu chí đánh giá: Minh bạch thông tin, tôn trọng quyền tác giả và liêm chính.`;
        } else if (/thực hành|báo cáo|dự án|rủi ro/i.test(combined)) {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 10; Chủ đề: A2; Mã chỉ báo NL AI: 10.A2.MR1\n- Yêu cầu cần đạt AI: Nêu được một số biện pháp hạn chế các rủi ro đối với con người, xã hội mà một sản phẩm AI có thể đem lại thông qua một dự án sáng tạo AI bài "${cleanLessonName}".\n- Hành vi học sinh: Lựa chọn công cụ AI phù hợp và phác thảo ý tưởng hoàn thành bài thực hành ${cleanLessonName}.\n- Sản phẩm đầu ra: Bản đề xuất ý tưởng giải quyết bài toán thực hành ${topicLabel} có sự hỗ trợ của AI.\n- Tiêu chí đánh giá: Ý tưởng sáng tạo, công cụ phù hợp và có tính khảthi cao.`;
        } else {
          row.aiCompetency2422Integrated = `Thành phần NL AI: NLa - Tư duy lấy con người làm trung tâm; Khối lớp: 10; Chủ đề: A1; Mã chỉ báo NL AI: 10.A1.1\n- Yêu cầu cần đạt AI: Nhận biết con người là chủ thể thiết kế, kiểm soát hoạt động và chịu trách nhiệm về quyết định của AI khi học "${cleanLessonName}".\n- Hành vi học sinh: Học sinh chủ động đóng vai trò người kiểm soát, ra quyết định cuối cùng khi sử dụng AI cho bài ${cleanLessonName}.\n- Sản phẩm đầu ra: Bản kết luận bài học ${topicLabel} do học sinh tự tổng hợp và quyết định.\n- Tiêu chí đánh giá: Khẳng định vai trò chủ thể của người học, làm chủ tri thức.`;
        }
      }
    }
  }
  ...
  if (isSocialMissing) {
    if (/biển|đảo|hải đảo|hoàng sa|trường sa|vùng biển|thềm lục địa|unclos|hàng hải|kinh tế biển/i.test(combined)) {
      row.socialIntegration = `- Mức độ: Tích hợp bộ phận\n- Chủ đề tích hợp: Giáo dục chủ quyền biển, đảo\n- Căn cứ YCCĐ: Khẳng định chủ quyền biển đảo Việt Nam (Hoàng Sa, Trường Sa) và khai thác bền vững tài nguyên biển ${topicLabel}\n- Nội dung tích hợp: Khẳng định chủ quyền lãnh thổ trên biển, bảo vệ an ninh biên giới biển đảo và khai thác kinh tế biển bền vững\n- Hành vi HS: Tìm hiểu tư liệu pháp lý (UNCLOS 1982, Luật Biển VN), phân tích bản đồ biển đảo và thiết kế tư liệu tuyên truyền\n- Sản phẩm & Minh chứng: Bản báo cáo/infographic số tuyên truyền chủ quyền biển đảo; tiêu chí đúng căn cứ pháp lý, khẳng định toàn vẹn lãnh thổ.`;
      ...
      row.socialIntegration = `- Mức độ: Tích hợp bộ phận\n- Chủ đề tích hợp: Giáo dục phòng, chống thiên tai\n- Căn cứ YCCĐ: Nhận diện quy luật thiên tai và rèn luyện kĩ năng ứng phó, giảm nhẹ rủi ro thảm họa ${topicLabel}\n- Nội dung tích hợp: Kĩ năng dự báo, nhận diện và các biện pháp phòng chống thiên tai (bão, lũ, hạn mặn, sạt lở) tại địa phương\n- Hành vi HS: Khai thác bản đồ cảnh báo thiên tai số, xây dựng kịch bản ứng phó và quy trình sơ tán an toàn\n- Sản phẩm & Minh chứng: Kế hoạch hành động/cẩm nang phòng chống thiên tai tại trường học và gia đình; tiêu chí phương án an toàn, khả thi, thao tác chuẩn xác.`;
      ...
      row.socialIntegration = `- Mức độ: Tích hợp bộ phận\n- Chủ đề tích hợp: Giáo dục ứng phó biến đổi khí hậu\n- Căn cứ YCCĐ: Phân tích tác động của biến đổi khí hậu và đề xuất giải pháp thích ứng, giảm phát thải ${topicLabel}\n- Nội dung tích hợp: Nguyên nhân, biểu hiện và các giải pháp thích ứng biến đổi khí hậu, chuyển dịch năng lượng xanh và giảm phát thải\n- Hành vi HS: Phân tích dữ liệu khí tượng, đề xuất mô hình thích ứng linh hoạt trong sản xuất và đời sống\n- Sản phẩm & Minh chứng: Bản cam kết hành động xanh thích ứng biến đổi khí hậu; tiêu chí nhận thức đúng đắn, giải pháp thiết thực.`;
      ...
      row.socialIntegration = `- Mức độ: Tích hợp bộ phận\n- Chủ đề tích hợp: Giáo dục bảo vệ môi trường\n- Căn cứ YCCĐ: Phân tích hiện trạng tài nguyên và đề xuất giải pháp bảo vệ môi trường, phát triển xanh ${topicLabel}\n- Nội dung tích hợp: Sử dụng hợp lý tài nguyên, bảo vệ đa dạng sinh học, kinh tế tuần hoàn và giảm thiểu rác thải\n- Hành vi HS: Thu thập dữ liệu thực tế, đề xuất các biện pháp bảo vệ tài nguyên và phân loại rác thải tại địa phương\n- Sản phẩm & Minh chứng: Sơ đồ tư duy/bản tin giải pháp bảo vệ môi trường và tăng trưởng xanh; tiêu chí giải pháp khả thi, gắn liền với thực tiễn.`;
    } else if (/dân số|dân cư|đô thị hóa|lao động|việc làm/i.test(combined)) {
      row.socialIntegration = `- Mức độ: Tích hợp bộ phận\n- Chủ đề tích hợp: Giáo dục dân số và phát triển bền vững\n- Căn cứ YCCĐ: Phân tích dữ liệu dân cư và tác động đến phát triển kinh tế - xã hội ${topicLabel}\n- Nội dung tích hợp: Cơ cấu dân số, lao động, việc làm và an sinh xã hội trong phát triển bền vững\n- Hành vi HS: Phân tích bảng số liệu dân cư, tháp tuổi và đề xuất giải pháp an sinh xã hội\n- Sản phẩm & Minh chứng: Báo cáo phân tích dân số và phát triển bền vững; tiêu chí số liệu chính xác, phân tích logic.`;
    } else if (/di sản|văn hóa|lễ hội|nghệ thuật|du lịch/i.test(combined)) {
      row.socialIntegration = `- Mức độ: Liên hệ / Lồng ghép\n- Chủ đề tích hợp: Giáo dục di sản văn hóa\n- Căn cứ YCCĐ: Nhận thức giá trị di sản và đề xuất giải pháp bảo tồn, phát huy gắn với phát triển du lịch ${topicLabel}\n- Nội dung tích hợp: Giá trị lịch sử văn hóa, ý thức bảo tồn di sản dân tộc và quảng bá du lịch\n- Hành vi HS: Khai thác tư liệu di sản, thiết kế ấn phẩm số quảng bá di sản\n- Sản phẩm & Minh chứng: Video/infographic quảng bá di sản văn hóa; tiêu chí thông tin chuẩn xác, tôn vinh giá trị văn hóa dân tộc.`;
    }
  }
